...
static void set_options(ObjectData* obj, struct Sass_Context *ctx) {
  ...
  if (!mapLink.empty()) {
  sass_option_set_source_map_file(opts, obj->o_get(mapLink, true, s_Sass).toStrRef());
  sass_option_set_omit_source_map_url(opts, false);
  sass_option_set_source_map_contents(opts, true);
  }
  String mapRoot = obj->o_get("map_root", true, s_Sass);
  if (!mapRoot.empty()) {
  sass_option_set_source_map_root(opts, obj->o_get(mapRoot, true, s_Sass).toStrRef());
  }
